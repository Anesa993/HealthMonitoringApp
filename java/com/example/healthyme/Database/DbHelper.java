...
//        db.close();
    ...
    // Deleting a specific Blood Pressure record by ID
    public boolean deleteBPRecord(int bpId) {
        SQLiteDatabase db = this.getWritableDatabase();
        int result = db.delete(TABLE_BP, KEY_BP_ID + " = ?", new String[]{String.valueOf(bpId)});
        db.close();
        return result > 0;
    }
