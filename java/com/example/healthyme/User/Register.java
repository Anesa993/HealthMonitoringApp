...
public class Register extends AppCompatActivity {
    ...
    TextInputEditText edtFullName, edtEmail, edtDOB, edtHeight, edtWeight, edtPassword,edtbmi;
    Button registerBtn;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        edtHeight.setOnFocusChangeListener(new View.OnFocusChangeListener() {
            @Override
            public void onFocusChange(View v, boolean hasFocus) {
                if (!hasFocus && !edtHeight.getText().toString().isEmpty() && !edtWeight.getText().toString().isEmpty()) {
                    calculateBMI();
                }
            }
        });
        ...
        if (!heightStr.isEmpty() && !weightStr.isEmpty()) {
            try {
                ...
                if (height > 0 && weight > 0) {
                    ...
                    if (edtbmi != null) {
                        edtbmi.setText(interpreteBMI(bmi));
                       // edtbmi.setText(bmi.tostring);
                       // edtbmi.setText(String.format("%.2f", bmi));
                    } else {
                        Log.e("Register", "edtbmi is null. Ensure it is initialized correctly.");
                    }
                } else {
                    edtbmi.setText(""); // Clear BMI if inputs are invalid
                    Toast.makeText(this, "Height and weight must be greater than zero.", Toast.LENGTH_SHORT).show();
                }
            } catch (NumberFormatException e) {
                Toast.makeText(this, "Invalid input for height or weight.", Toast.LENGTH_SHORT).show();
            }
        } else {
            edtbmi.setText(""); // Clear BMI if inputs are incomplete
        }
    }
