...
public class DataBaseHelper extends SQLiteOpenHelper {
    ...
    */
    ...
    public int getCalories(String date) {
        SQLiteDatabase db = this.getReadableDatabase(); // Get database for reading
        int calories = 0; // Default value in case no data is found

        // Create cursor to query the table and find the entry that matches the date
        Cursor cursor = db.query(TABLE_NUTRITION,
                new String[]{KEY_CALORIES, KEY_PROTEIN, KEY_FAT, KEY_CARBS, KEY_WEIGHT},
                KEY_DATE + " =?",
                new String[]{date},
                null, null, null, null);

        // Check if cursor is not null and contains at least one result
        if (cursor != null) {
            if (cursor.moveToFirst()) {
                calories = cursor.getInt(cursor.getColumnIndex(KEY_CALORIES));
            }
            cursor.close(); // Always close the cursor to avoid memory leaks
        }

        return calories; // Return the calories value or 0 if no data was found
    }
    ...
    public int getProtein(String date) {
        SQLiteDatabase db = this.getReadableDatabase(); // Get database for reading
        int protein = 0; // Default value in case no data is found

        // Create cursor to query the table and find the entry that matches the date
        Cursor cursor = db.query(TABLE_NUTRITION,
                new String[]{KEY_PROTEIN}, // Only selecting the KEY_PROTEIN column
                KEY_DATE + " =?",
                new String[]{date},
                null, null, null, null);
        ...
        if (cursor != null) {
            if (cursor.moveToFirst()) {
                protein = cursor.getInt(cursor.getColumnIndex(KEY_PROTEIN));
            }
            cursor.close(); // Always close the cursor to avoid memory leaks
        }

        return protein; // Return the protein value or 0 if no data was found
    }
    ...
    public int getFat(String date) {
        SQLiteDatabase db = this.getReadableDatabase(); // Get database for reading
        int fat = 0; // Default value in case no data is found

        // Create cursor to query the table and find the entry that matches the date
        Cursor cursor = db.query(TABLE_NUTRITION,
                new String[]{KEY_FAT}, // Only selecting the KEY_FAT column
                KEY_DATE + " =?",
                new String[]{date},
                null, null, null, null);
        ...
        if (cursor != null) {
            if (cursor.moveToFirst()) {
                fat = cursor.getInt(cursor.getColumnIndex(KEY_FAT));
            }
            cursor.close(); // Always close the cursor to avoid memory leaks
        }

        return fat; // Return the fat value or 0 if no data was found
    }
    ...
    public int getCarbs(String date) {
        SQLiteDatabase db = this.getReadableDatabase(); // Get database for reading
        int carbs = 0; // Default value in case no data is found

        // Create cursor to query the table and find the entry that matches the date
        Cursor cursor = db.query(TABLE_NUTRITION,
                new String[]{KEY_CARBS}, // Only selecting the KEY_CARBS column
                KEY_DATE + " =?",
                new String[]{date},
                null, null, null, null);
        ...
        if (cursor != null) {
            if (cursor.moveToFirst()) {
                carbs = cursor.getInt(cursor.getColumnIndex(KEY_CARBS));
            }
            cursor.close(); // Always close the cursor to avoid memory leaks
        }

        return carbs; // Return the carbs value or 0 if no data was found
    }
    ...
    public int getWeight(String date) {
        SQLiteDatabase db = this.getReadableDatabase(); // Get database for reading
        int weight = 0; // Default value in case no data is found

        // Create cursor to query the table and find the entry that matches the date
        Cursor cursor = db.query(TABLE_NUTRITION,
                new String[]{KEY_WEIGHT}, // Only selecting the KEY_WEIGHT column
                KEY_DATE + " =?",
                new String[]{date},
                null, null, null, null);
        ...
        if (cursor != null) {
            if (cursor.moveToFirst()) {
                weight = cursor.getInt(cursor.getColumnIndex(KEY_WEIGHT));
            }
            cursor.close(); // Always close the cursor to avoid memory leaks
        }

        return weight; // Return the weight value or 0 if no data was found
    }
